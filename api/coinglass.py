...
# .6/.4 (Huobi/Kraken) return v=0 — use Binance(.A), Coinbase(.C), Bybit(.3), OKX(.2)
...
# Coinalyze keeps max ~1500-2000 intraday points. 1H: ~62 days, 4H: ~250 days. Daily: unlimited.
# 7D uses daily interval and aggregates to weekly on frontend.
...
def get_live_oi_weights():
    ...
    try:
        ...
        if total > 0:
            weights = {k: v / total for k, v in oi_map.items()}
        else:
            weights = {}
...
def funding_by_interval(interval_key):
    cache_key = "funding_iv7_" + interval_key  # v4: 7D=weekly, r field, MA10
    cached = cache_read(cache_key)
    if cached: return cached
    ...
    # Fetch live OI weights for proper OI-weighted average
    oi_weights = get_live_oi_weights()  # {symbol: normalised_weight}
    ...
    for sd in (data if isinstance(data, list) else []):
        ...
        for c in sd.get("history", []):
            t      = c.get("t", 0)
            val    = float(c.get("c", 0))  # rate is in c (close) per API docs
    ...
    ppd = {"1H": 24.0, "4H": 6.0, "1D": 3.0, "7D": 3.0}.get(interval_key, 3.0)  # payments per day
...
def funding_legacy(days):
    ...
    for sd in (data if isinstance(data, list) else []):
        for c in sd.get("history", []):
            t   = c.get("t", 0)
            val = float(c.get("c", 0))  # funding-rate-history: rate is in c (close) per API docs
...
def cvd_data(days=None, interval_key='1D'):
    ...
    - 1H: 1hour bars, ~60 days (Coinalyze intraday limit ~1500 pts)
    - 4H: 4hour bars, ~250 days
...
def long_short_ratio(days):
    """
    Fetch BTC long/short ratio from Coinalyze.
    Uses BTCUSDT_PERP.A (Binance) as primary — most liquid and longest history.
    Falls back to multi-symbol average if needed.
    Coinalyze stores full history back to ~2019.
    """
    ...
    # Try primary: Binance perp (longest history on Coinalyze)
    ...
    for sd in (data if isinstance(data, list) else []):
        for c in sd.get("history", []):
            t  = c.get("t", 0)
            ls = c.get("l", None)   # long ratio (0-1 float)
            if ls is not None:
                by_time[t] = float(ls)

    # If very sparse, also pull other perps and average
    if len(by_time) < 30:
    ...
    for t in sorted(by_time.keys()):
        long_pct = by_time[t]
        # l = long ratio (0-1). L/S ratio = long_pct / (1 - long_pct)
...
def btc_price_data():
    """Fetch and cache full BTC price history from CoinMetrics community API."""
    cache_key = "btc_price"
    cached = cache_read(cache_key)
    if cached: return cached
...
def oi_history(interval_key='1D'):
    """Fetch aggregated BTC OI from Coinalyze. Supports 1H/4H/1D intervals."""
    iv_map = {'1H':'1hour','4H':'4hour','1D':'daily'}
    ...
    result = [{"time": datetime.fromtimestamp(t, tz=timezone.utc).strftime(fmt), "value": by_ts[t]} for t in sorted(by_ts)]
...
def coinglass_get(path, params):
    """HTTP GET to CoinGlass v4 API."""
    from urllib.parse import urlencode
    url = "https://open-api-v4.coinglass.com/api/" + path + "?" + urlencode(params)
...
def coinglass_cvd_data():
    """
    Fetch aggregated CVD from CoinGlass:
    - Perp CVD:  /futures/aggregated-cvd/history  (all exchanges, BTC, 1D bars)
    - Spot CVD:  /spot/aggregated-cvd/history      (all exchanges, BTC, 1D bars)
    Returns cumulative series in same format as old cvd_data().
    """
    cache_key = "cg_cvd_v1_1D"
    cached = cache_read(cache_key)
    if cached:
        return cached

    now_ms  = int(time.time() * 1000)
    # CoinGlass max history ~3 years daily; use 2000d
    frm_ms  = now_ms - 86400 * 2000 * 1000

    # ── Perp CVD (futures aggregated across all exchanges) ─────────────────
    perp_resp = coinglass_get("futures/aggregated-cvd/history", {
        "symbol":    "BTC",
        "interval":  "1d",
        "startTime": frm_ms,
        "endTime":   now_ms
    })
    # ── Spot CVD (spot aggregated across all exchanges) ────────────────────
    spot_resp = coinglass_get("spot/aggregated-cvd/history", {
        "symbol":    "BTC",
        "interval":  "1d",
        "startTime": frm_ms,
        "endTime":   now_ms
    ...
    def build_cumulative(resp):
        rows = resp.get("data") or []
        cum = 0.0
        result = []
        for row in rows:
            # timestamp in ms
            t_ms = row.get("time") or row.get("t") or 0
            if t_ms > 1e12:
                t_ms = int(t_ms)
            else:
                t_ms = int(t_ms) * 1000
            delta = float(row.get("cum_vol_delta") or 0)
            # CoinGlass returns per-bar delta; accumulate
            cum += delta
            iso = datetime.fromtimestamp(t_ms / 1000, tz=timezone.utc).strftime("%Y-%m-%d")
            result.append({"time": iso, "cvd": cum})
        return result

    perp_series = build_cumulative(perp_resp)
    spot_series = build_cumulative(spot_resp)

    perp_map = {x["time"]: x["cvd"] for x in perp_series}
    spot_map = {x["time"]: x["cvd"] for x in spot_series}
    all_times = sorted(set(list(perp_map) + list(spot_map)))

    result = [{
        "time":     t,
        "spot_cvd": spot_map.get(t, 0.0),
        "perp_cvd": perp_map.get(t, 0.0)
    } for t in all_times]

    latest = result[-1] if result else {}
    out = {"data": result, "latest": latest, "interval": "1D", "source": "coinglass"}
    cache_write(cache_key, out)
    return out
    ...
    def do_GET(self):
        ...
        try:
            if metric == "funding":
                ...
                try:
                    result = coinglass_cvd_data()
                    cache_key = "cg_cvd_v1_1D"
                except Exception as cg_err:
                    # Fallback to Coinalyze if CoinGlass fails
